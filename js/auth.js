...
function initAuthPage() {
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch('https://wordmaze.nasiuduklapangantenis.my.id/api/login', {
        method: 'POST',
        headers: {
  ...
  registerForm.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch('https://wordmaze.nasiuduklapangantenis.my.id/api/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
